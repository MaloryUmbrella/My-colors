...
export function onNextPalette(props, props2, i, ids) {
  if (props.state.locked[i - 1]) {
    ...
    if (statePreviousLock.getAttribute("data-icon") === "lock") {
      ...
      let nextSpanColorFormat = document.getElementById("colorFormat-" + (i + 1));
      let previousDivColor = document.getElementById("div" + i);
      let previousInputColorDisplay = document.getElementById("input" + ids);
      let previousSpanColorFormat = document.getElementById("colorFormat-" + (i));

      let nextSpanTextValue = nextSpanColorFormat.innerText;
      let previousSpanTextValue = previousSpanColorFormat.innerText;

      if (previousSpanTextValue !== nextSpanTextValue) {
        previousSpanColorFormat.innerText = nextSpanTextValue;
        nextSpanColorFormat.innerText = previousSpanTextValue;
      }

      let backgroundColor = previousDivColor.style.backgroundColor;

      let temp = nextSpanTextValue;
      nextSpanTextValue = previousSpanTextValue;
      previousSpanTextValue = temp;

      if (nextSpanTextValue === "HEX") {
      ...
      else if (nextSpanTextValue === "RGB") {
        nextInputColorDisplay.value = noSpaces(backgroundColor.substring(4, (backgroundColor.length - 1)));
      }
      else {
        nextInputColorDisplay.value = noSpaces(rgbToHSL(backgroundColor.substring(4, (backgroundColor.length - 1))));
      }
      
      backgroundColor = nextDivColor.style.backgroundColor;

      if (previousSpanTextValue === "HEX") {
      ...
      else if (previousSpanTextValue === "RGB") {
        previousInputColorDisplay.value = noSpaces(backgroundColor.substring(4, (backgroundColor.length - 1)));
      }
      else {
        previousInputColorDisplay.value = noSpaces(rgbToHSL(backgroundColor.substring(4, (backgroundColor.length - 1))));
      }
...
export function changeColor(color, div, props, input) {
  ...
  if (color.length === 6) {
    ...
    let thisSpanColorFormat = document.getElementById("colorFormat-" + div);
    let thisSpanTextValue = thisSpanColorFormat.innerText;
    let thisInputColorDisplay = document.getElementById("input" + input);

    if (thisSpanTextValue === "RGB") {
      color = hexToRGB(colorWithoutHash(color).toUpperCase());
    }
    else if (thisSpanTextValue === "HSL") {
      color = rgbToHSL(hexToRGB(colorWithoutHash(color).toUpperCase()));
    }

    if (thisSpanTextValue !== "HEX") {
      thisInputColorDisplay.value = color;
    }
